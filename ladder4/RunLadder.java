...
public class RunLadder {
	...
	void runLadder(int selectedPlayerNumber) {
		...
		CurrentPoint cp = new CurrentPoint(selectedPlayerNumber - 1, 0);
		...
		while (cp.getPointY() < columnLength) {
			...
			if (cp.getPointY() > columnLength - 1) {// 사다리의 맨 밑을 벗어나면 중단
				...
				if (ladderAL.get(cp.getPointX())[cp.getPointY()] == 1) {
					cp.setPointX(cp.getPointX() + 1);
				} else if (ladderAL.get(cp.getPointX())[cp.getPointY()] == -1) {
					cp.setPointX(cp.getPointX() - 1);
				}
				lastMovedDirection = Direction.HOIZENTAL;
			}
	...
	void displayCurrentPoint(CurrentPoint cp) {
		System.out.println("enter displayCurrentPoint");
		...
		System.out.println("out displayCurrentPoint");
	}
}
